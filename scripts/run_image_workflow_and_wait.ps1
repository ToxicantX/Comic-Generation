...
if (-not $SubmitScript) { $SubmitScript = Join-Path $PSScriptRoot "submit_image_workflow.ps1" }
if (-not $WaitScript) { $WaitScript = Join-Path $PSScriptRoot "wait_comfy_prompt.ps1" }
if (-not $ResultPath) { $ResultPath = Join-Path $comicConfig.Workspace "manifests\image_workflow_result.json" }
